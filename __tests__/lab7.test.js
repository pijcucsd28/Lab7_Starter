describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allSayRemove = await page.evaluate(() => {
      const items = document.querySelectorAll('product-item');
      for (let i = 0; i < items.length; i++) {
        const button = items[i].shadowRoot.querySelector('button');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    let allHaveAddText = true;

    const productItems = await page.$$('product-item');
    for (let i = 0; i < productItems.length; i++) {
      const text = await productItems[i].evaluate(item => item.shadowRoot.querySelector('button').innerText);
      if (text !== 'Add to Cart') {
        allHaveAddText = false;
      }
    }
    expect(allHaveAddText).toBe(true);
